...
{{snippet:guide-c-developers}}

C developers often bring plain functions, structs, pointers, headers, and ABI boundaries.
Musi keeps the useful part of that experience, but the code shape changes: Use safe Musi values first, then cross C with `foreign`, `unsafe`, and `@std/ffi` pointer views.

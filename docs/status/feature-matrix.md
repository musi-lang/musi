...
| Exports and opaque modules                         | ✅         | ✅           | ✅    | ✅   | ✅         | Export collection, opaque marking, and SEAM metadata emission exist; runtime hiding remains out of scope                                                                                                                                      |
...
| `@repr` and `@layout` attrs         | ✅         | ✅           | ✅    | ✅   | ✅         | Layout-sensitive metadata is carried into SEAM artifacts (`.data` descriptors); runtime ABI execution remains out of scope                             |
...
| Sema-to-IR lowering (`music_ir`)                 | ✅      | IR covers multi-index access, dynamic imports, syntax values, first-class type values, record case patterns, and local recursive callables without unsupported placeholder IR                              |
...
| `musi_native` crate                      | 🟡      | `crates/musi_native` now owns the repo-owned host/world boundary behind the runtime path, including registered foreign/effect handlers, `musi:test` report collection, cfg-selected desktop dispatch, and optional fallback delegation; broader platform capability coverage remains in progress |
...
| Runtime execution for quote/splice       | ✅      | `musi_vm` keeps syntax as runtime data. `musi_rt` executes syntax through expression/module compilation services. VM-native staged execution is out of scope                                                                                                                                     |
...
| `musi:*` foundation namespace | 🟡      | `musi:test` is the implemented foundation root for package-test events, and `musi_foundation` now owns the canonical Rust-side registry for current `musi:*` modules; the broader source-visible foundation inventory is not complete yet |

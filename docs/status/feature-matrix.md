...
| SEAM runtime execution / VM              | 🟡      | Core emitted programs now execute through `musi_vm` (globals, calls, closures, records, sequences, type ops, eager `mod.load`, handled effects, `resume`); host foreign/effect callbacks now receive typed signature metadata, while broader runtime-engine work remains |
...
| Runtime execution for quote/splice       | 🟡      | `musi_rt` now compiles typed expression syntax and module syntax through `music_session`; VM-native staged execution still does not exist                                                                                                                                |

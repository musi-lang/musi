...
| `musi_native` crate                      | 🟡      | `crates/musi_native` now provides the repo-owned host/world integration layer behind the default runtime path, with registered foreign/effect handlers and fallback host delegation; broader official target coverage remains in progress |
...
| `musi:*` foundation namespace | 🟡      | `musi:test` is now the implemented foundation root for package-test events; the broader source-visible foundation that replaces hidden compiler/package magic is not complete yet                                       |

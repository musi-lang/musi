...
| `musi_native` crate                      | 🟡      | `crates/musi_native` now provides the repo-owned default host adapter layer for registered foreign/effect handlers and fallback host delegation; broader OS/libc coverage remains in progress              |
...
| `musi:*` intrinsic namespace | 🟡      | `musi:test` is now the implemented foundation for package-test events; broader host/runtime capability roots still need expansion                                                                                       |

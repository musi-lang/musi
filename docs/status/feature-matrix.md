...
| SEAM runtime execution / VM              | 🟡      | Core emitted programs now execute through `musi_vm` (globals, calls, closures, records, sequences, type ops, eager `mod.load`, handled effects, `resume`); host-owned foreign calls and broader runtime-engine work remain              |

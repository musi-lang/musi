...
let validate_utf8_char state pos =
  if pos >= state.len then (pos, None)
    ...
    let first_byte = Char.code state.source.[pos] in
    if first_byte < 0x80 then (pos + 1, None) (* ASCII char  *)
    else if first_byte < 0xC0 then
      (pos + 1, Some ("invalid UTF-8 continuation byte", pos, pos + 1))
    else if first_byte < 0xE0 then
      (* 2-byte seq *)
      if pos + 1 < state.len then
        let second_byte = Char.code state.source.[pos + 1] in
        if second_byte land 0xC0 = 0x80 then (pos + 2, None)
        else
          (pos + 2, Some ("invalid UTF-8 continuation byte", pos + 1, pos + 2))
      else (state.len, Some ("incomplete UTF-8 sequence", pos, state.len))
    else if first_byte < 0xF0 then
      (* 3-byte seq *)
      if pos + 2 < state.len then
        let second_byte = Char.code state.source.[pos + 1] in
        let third_byte = Char.code state.source.[pos + 2] in
        if second_byte land 0xC0 = 0x80 && third_byte land 0xC0 = 0x80 then
          (pos + 3, None)
        else
          let invalid_pos =
            if second_byte land 0xC0 <> 0x80 then pos + 1 else pos + 2
          in
          ( pos + 3
          , Some
              ("invalid UTF-8 continuation byte", invalid_pos, invalid_pos + 1)
          )
      else (state.len, Some ("incomplete UTF-8 sequence", pos, state.len))
    else if first_byte < 0xF8 then
      (* 4-byte sequence *)
      if pos + 3 < state.len then
        let second_byte = Char.code state.source.[pos + 1] in
        let third_byte = Char.code state.source.[pos + 2] in
        let fourth_byte = Char.code state.source.[pos + 3] in
        if
          second_byte land 0xC0 = 0x80
          && third_byte land 0xC0 = 0x80
          && fourth_byte land 0xC0 = 0x80
        then (pos + 4, None)
        else
          let invalid_pos =
            if second_byte land 0xC0 <> 0x80 then pos + 1
            else if third_byte land 0xC0 <> 0x80 then pos + 2
            else pos + 3
          in
          ( pos + 4
          , Some
    ...
    if pos >= len then ()
      ...
      | '\\' when pos + 1 < len -> (
        ...
        | ('u' | 'U') when pos + 3 < len && content.[pos + 2] = '{' -> (
          let rec find_end_brace p =
            if p >= len then None
            else if content.[p] = '}' then Some p
            else if is_xdigit content.[p] then find_end_brace (p + 1)
            else None
          in
          match find_end_brace (pos + 3) with
          | Some end_brace -> (
            let hex_content =
...
let validate_template_braces state _open_braces extra_closing =
  let final_state = ref state in
...
let parse_unicode_escape state pos =
  ...
  then
    ...
    if pos + 3 < state.len && state.source.[pos + 2] = '{' then
      let rec find_end_brace p =
        if p >= state.len then None
        else if state.source.[p] = '}' then Some p
        else if is_xdigit state.source.[p] then find_end_brace (p + 1)
        else None
      in
      match find_end_brace (pos + 3) with
      | Some end_brace -> (
...
let scan_template state =
  let start_pos = state.pos in
  let end_pos, is_unterminated, open_braces, extra_closing =
    scan_template_content state
  in
  let span = make_span state start_pos in
  let state_with_brace_errors =
    validate_template_braces state open_braces extra_closing
  in

...
let scan_whitespace state =
  let end_pos =
    scan_while state state.pos (function
      | ' ' | '\t' | '\r' -> true
      | _ -> false)
  in
...
let scan_ident state =
  ...
  let rec loop pos acc_state =
    if pos >= state.len then (pos, acc_state)
      ...
      match state.source.[pos] with
      | 'a' .. 'z' | 'A' .. 'Z' | '0' .. '9' | '_' -> loop (pos + 1) acc_state
...
let rec scan_number state =
  ...
  if state.pos + 1 < state.len && state.source.[state.pos] = '0' then
    ...
    | 'x' | 'X' -> scan_hex_number state start_pos
    | 'b' | 'B' -> scan_binary_number state start_pos
    | '0' .. '9' ->
      let error_state =
...
and scan_hex_number state start_pos =
  let pos = state.pos + 2 in
  (* Skip '0x' *)
  if pos >= state.len then
    let span = make_span state start_pos in
    let error_state = add_error state "incomplete hex number" span in
    ({ error_state with pos }, "0x", span)
  else
    let rec loop p acc_state =
      if p >= state.len then (p, acc_state)
      else
        match state.source.[p] with
        | '0' .. '9' | 'a' .. 'f' | 'A' .. 'F' -> loop (p + 1) acc_state
        | c ->
          let char_span = Span.make state.file_id p (p + 1) in
          let msg = Printf.sprintf "invalid hex digit '%c'" c in
          (p, add_error acc_state msg char_span)
    in
    let end_pos, final_state = loop pos state in
    if end_pos = pos then
      let span = make_span state start_pos in
      let error_state = add_error final_state "incomplete hex number" span in
...
and scan_binary_number state start_pos =
  let pos = state.pos + 2 in
  (* skip '0b' *)
  if pos >= state.len then
    let span = make_span state start_pos in
    let error_state = add_error state "incomplete binary number" span in
    ({ error_state with pos }, "0b", span)
  else
    let rec loop p acc_state =
      if p >= state.len then (p, acc_state)
      else
        match state.source.[p] with
        | '0' | '1' -> loop (p + 1) acc_state
        | c ->
          let char_span = Span.make state.file_id p (p + 1) in
          let msg = Printf.sprintf "invalid binary digit '%c'" c in
          (p, add_error acc_state msg char_span)
    in
    let end_pos, final_state = loop pos state in
    if end_pos = pos then
      let span = make_span state start_pos in
      let error_state = add_error final_state "incomplete binary number" span in
      ( { error_state with pos = end_pos }
      , extract_content state start_pos end_pos
      , span )
    else
    ...
    if pos >= state.len then (pos, acc_state, dot_count)
      ...
      | '.' ->
        if dot_count > 0 then
          ...
          in
          loop (pos + 1) error_state (dot_count + 1)
        else loop (pos + 1) acc_state (dot_count + 1)
      | _ -> (pos, acc_state, dot_count)
  in
...
let scan_string state =
  let start_pos = state.pos in
  let end_pos = scan_quoted_content state '"' in
  let span = make_span state start_pos in
...
let scan_rune state =
  ...
  with
  | Some error_state -> ({ error_state with pos = end_pos }, '\000', span)
  | None ->
    let content = extract_content state (state.pos + 1) (end_pos - 1) in

    if String.length content = 0 then
      let error_state = add_error state "empty rune literal" span in
      ({ error_state with pos = end_pos }, '\000', span)
    else if
    ...
    then
      ...
      in
      ({ error_state with pos = end_pos }, content.[0], span)
    else
      let final_state, final_char =
        if String.length content = 2 && content.[0] = '\\' then
          if not (validate_escape_sequence state (state.pos + 1)) then
            let msg =
          ...
          let error_state =
            add_error state "unterminated escape sequence" span
          in
          (error_state, '\000')
      in
      ({ final_state with pos = end_pos }, final_char, span)
...
let lex_token state =
  ...
  | Some c -> (
    ...
    if char_code < 32 && c != '\t' && c != '\n' && c != '\r' then
      ...
      | '/' when state.pos + 1 < state.len && state.source.[state.pos + 1] = '/'
        ->
...
let tokenize source file_id interner =
  let rec loop state tokens =
    ...
    | _ -> loop new_state ((token, span) :: tokens)
  in

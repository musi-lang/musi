use super::{
    HirExprId, HirPatId, HirPatKind, HirRecordPatField, HirTyId, HirTyKind, Ident, IrExpr,
    IrExprKind, IrLit, IrOrigin, LowerCtx, ModuleKey, SliceRange, decl_binding_id, fresh_temp,
    lower_expr, lowering_invariant_violation, record_layout_for_ty,
};

pub(crate) type RecordPatFieldRange = SliceRange<HirRecordPatField>;

#[derive(Clone, Copy)]
pub(crate) struct IrrefutablePatInput<'a> {
    origin: IrOrigin,
...
pub(crate) fn lower_destructure_let(
    ctx: &mut LowerCtx<'_>,
    origin: IrOrigin,
    pat: HirPatId,
    value: HirExprId,
...
pub(crate) fn project_index_expr(origin: IrOrigin, base: IrExpr, index: usize) -> Option<IrExpr> {

use super::{
    DefinitionKey, HirExprId, HirHandleClause, HirTyKind, Ident, IrExpr, IrExprKind, IrHandleOp,
    IrOrigin, LowerCtx, LoweringResult, SliceRange, call, lower_expr, lowering_invariant_violation,
};

pub(crate) fn lower_request_expr(ctx: &mut LowerCtx<'_>, expr: HirExprId) -> LoweringResult {
    call::lower_request_expr(ctx, expr)
}

pub(crate) fn lower_answer_literal_expr(
    ctx: &mut LowerCtx<'_>,
    expr_id: HirExprId,
    effect_name: Ident,
...
pub(crate) fn lower_handle_expr(
    ctx: &mut LowerCtx<'_>,
    expr_id: HirExprId,
    expr: HirExprId,
    answer: HirExprId,
...
pub(crate) fn lower_answer_effect_key(ctx: &LowerCtx<'_>, answer: HirExprId) -> DefinitionKey {
    let sema = ctx.sema;
    let answer_ty = sema
        .try_expr_ty(answer)
...
pub(crate) fn lower_answer_clause_closure(
    ctx: &mut LowerCtx<'_>,
    origin: IrOrigin,
    prefix: &str,
    params: &[Ident],

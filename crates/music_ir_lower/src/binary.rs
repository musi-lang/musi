use super::{
    HirBinaryOp, HirExprId, HirTyKind, Interner, IrBinaryOp, LowerCtx, lowering_invariant_violation,
};
...
pub(crate) fn lower_logical_binary_op(
    op: &HirBinaryOp,
    left_ty: &HirTyKind,
    interner: &Interner,
) -> Option<IrBinaryOp> {
...
pub(crate) fn is_bits_ty(ty: &HirTyKind, interner: &Interner) -> bool {
    match ty {
        HirTyKind::Bits { .. } => true,

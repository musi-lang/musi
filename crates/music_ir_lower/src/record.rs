use super::{
    BTreeMap, HirExprId, HirRecordItem, HirTyId, HirTyKind, Interner, IrExpr, IrExprKind, IrOrigin,
    IrRecordField, IrRecordLayoutField, LowerCtx, SemaModule, SliceRange, fresh_temp, lower_expr,
    lowering_invariant_violation, qualified_name, record_layout_for_ty, render_ty_name,
};

pub(crate) type RecordItemRange = SliceRange<HirRecordItem>;

pub(crate) fn record_storage_ty_name(
    sema: &SemaModule,
    ty: HirTyId,
    interner: &Interner,
) -> Box<str> {
...
pub(crate) struct RecordUpdateLayout<'a> {
    result_ty: HirTyId,
...
pub(crate) fn to_ir_origin(sema: &SemaModule, expr_id: HirExprId) -> IrOrigin {
    let origin = sema.module().store.exprs.get(expr_id).origin;
    IrOrigin::new(origin.source_id, origin.span)
}

pub(crate) type RecordSourceMap = BTreeMap<Box<str>, IrExpr>;
pub(crate) type RecordSourceResult = Result<(Vec<IrExpr>, RecordSourceMap), Box<str>>;

pub(crate) fn collect_record_sources(
    ctx: &mut LowerCtx<'_>,
    origin: IrOrigin,
    items: RecordItemRange,
...
pub(crate) fn lower_ordered_record_fields(
    _interner: &Interner,
    field_count: u16,
    indices: &BTreeMap<Box<str>, u16>,
...
pub(crate) fn lower_record_update_without_spread(
    ctx: &mut LowerCtx<'_>,
    base: HirExprId,
...
pub(crate) fn lower_record_update_with_spread(
    ctx: &mut LowerCtx<'_>,
    expr_id: HirExprId,
    base: HirExprId,
    update_items: &[HirRecordItem],

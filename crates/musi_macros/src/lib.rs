...
use syn::{
    Attribute, FnArg, Ident, ItemMod, LitStr, Pat, ReturnType, Type, parse_macro_input,
    spanned::Spanned,
};
...
fn generate(
    module: &mut ItemMod,
    specifier_override: Option<String>,
    ...
    for spec in &specs {
        ...
        let src = if let Some(sig) = &spec.musi_src {
            ...
            format!(
                "#[intrinsic(\"{fn_name_str}\")] export extrin fn {fn_name_str}({params_str}): {ret_type};\n"
            )
        };

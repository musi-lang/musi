...
impl Vm {
    ...
    pub(crate) fn exec_runtime_kernel_i64_arg(
        &self,
        module_slot: usize,
        kernel: RuntimeKernel,
        arg: i64,
        ...
        match kernel {
            ...
            | RuntimeKernel::DataConstructMatchAdd { source: 0, smi } => Ok(Some(
                arg.checked_add(i64::from(smi))
                    .ok_or_else(int_overflow_error)?,
            )),
            RuntimeKernel::DirectIntWrapperCall {
                arg_local: 0,
                const_arg,
                procedure,
            } => self.exec_direct_int_wrapper_kernel(module_slot, procedure, arg, const_arg),
            _ => Ok(None),
        }
    }

    pub(crate) fn exec_runtime_kernel_seq_arg(
        &mut self,
        kernel: RuntimeKernel,
        arg: GcRef,
    ) -> VmResult<Option<i64>> {
        match kernel {
            RuntimeKernel::Seq2Mutation2x2 {
                ...
                .heap
                .fast_seq2_mutation_2x2_kernel(arg, init_value, update_add)
                .map(Some),
            RuntimeKernel::Seq2Mutation(plan) if plan.grid_local == 0 => {
                self.heap.fast_seq2_mutation(arg, plan).map(Some)
            }
            _ => Ok(None),
        }
    }

    fn exec_direct_int_wrapper_kernel(
        &self,
        module_slot: usize,
        procedure: ProcedureId,
        arg: i64,
        ...
        let loaded = self
            ...
            .program
            .loaded_procedure(procedure)?;
        let Some(kernel) = loaded.runtime_kernel() else {
            return Ok(None);
        };
        match kernel {
            RuntimeKernel::IntTailAccumulator {
                ...
                if !set_kernel_local_i64(&mut locals, compare_local, arg, i64::from(const_arg))
                    || !set_kernel_local_i64(&mut locals, dec_local, arg, i64::from(const_arg))
                    || !set_kernel_local_i64(&mut locals, acc_local, arg, i64::from(const_arg))
                    || !set_kernel_local_i64(&mut locals, add_local, arg, i64::from(const_arg))
                    || usize::from(return_local) >= locals.len()
                {
                    return Ok(None);
                }
                let kernel = IntTailAccumulatorKernel {
                    compare_local,
                    compare_smi,
                    compare,
                    dec_local,
                ...
                loop {
                    if kernel.compare_local != kernel.dec_local {
                        locals[usize::from(kernel.compare_local)] =
                            locals[usize::from(kernel.dec_local)];
                    }
                    let compare_value = locals[usize::from(kernel.compare_local)];
                    if compare_int_for_kernel(
                        kernel.compare,
                        compare_value,
                        i64::from(kernel.compare_smi),
                    ) {
                        return Ok(Some(locals[usize::from(kernel.return_local)]));
                    }
...
const fn set_kernel_local_i64(locals: &mut [i64; 2], slot: u16, first: i64, second: i64) -> bool {
    match slot {
        0 => {
            locals[0] = first;
            true

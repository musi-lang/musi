...
macro_rules! binary_op {
    ...
    ($frame:expr, nat $method:ident) => {{
        let (b, a) = $frame.pop2()?;
        $frame.stack.push(Value::from_nat(a.as_nat()?.$method(b.as_nat()?)));
    }};
    ...
    ($frame:expr, cmp_uint $op:tt) => {{
        let (b, a) = $frame.pop2()?;
        $frame.stack.push(Value::from_bool(a.as_nat()? $op b.as_nat()?));
    }};
...
macro_rules! div_op {
    ...
    ($frame:expr, nat $op:tt) => {{
        let (b, a) = $frame.pop2()?;
        let divisor = b.as_nat()?;
        if divisor == 0 { return Err(VmError::DivideByZero); }
        $frame.stack.push(Value::from_nat(a.as_nat()? $op divisor));
    }};
}
...
pub fn exec(op: Opcode, frame: &mut Frame) -> Result<bool, VmError> {
    match op {
        ...
        Opcode::I_ADD_UN => binary_op!(frame, nat wrapping_add),
        Opcode::I_SUB => binary_op!(frame, int wrapping_sub),
        Opcode::I_SUB_UN => binary_op!(frame, nat wrapping_sub),
        Opcode::I_MUL => binary_op!(frame, int wrapping_mul),
        Opcode::I_MUL_UN => binary_op!(frame, nat wrapping_mul),
        Opcode::I_DIV => div_op!(frame, int wrapping_div),
        Opcode::I_DIV_UN => div_op!(frame, nat /),
        Opcode::I_REM => div_op!(frame, int wrapping_rem),
        Opcode::I_REM_UN => div_op!(frame, nat %),
        Opcode::I_NEG => {
            let a = frame.pop()?;
            frame
                .stack

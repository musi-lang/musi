...
use crate::cli::{
    Command, DiagnosticsFormatArg, FmtArgs, FmtMatchArmArrowAlignmentArg, FmtMatchArmIndentArg,
    FmtOperatorBreakArg, FmtProfileArg,
};
use crate::error::{MusiError, MusiResult};
use crate::workspace::{selected_package_entries, selected_package_roots, uses_workspace_scope};
use musi_fmt::{
...
pub fn init_package(target: Option<&Path>) -> MusiResult {
...
pub fn test_project(target: Option<&Path>, workspace: u8) -> MusiResult {
    reject_workspace_target(workspace, target)?;
    let anchor = project_anchor(target)?;
    let project = load_project_ancestor(anchor, ProjectOptions::default())?;
...
pub fn run_task(name: &str, target: Option<&Path>) -> MusiResult {
    let anchor = project_anchor(target)?;
    let project = load_project_ancestor(anchor, ProjectOptions::default())?;
...
pub fn install_project(target: Option<&Path>) -> MusiResult {
    let anchor = project_anchor(target)?;
    let project = load_project_ancestor(anchor, ProjectOptions::default())?;
...
pub fn print_project_metadata(target: Option<&Path>) -> MusiResult {
    let anchor = project_anchor(target)?;
    let project = load_project_ancestor(anchor, ProjectOptions::default())?;
    let entry = resolve_project_entry(&project, target)?;
...
pub fn fmt_project(args: &FmtArgs) -> MusiResult {
...
pub fn reserved_command_for(command: Command) -> MusiResult {

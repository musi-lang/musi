...
///
/// The entry point is determined from `mspackage.toml` (`package.main`) or
...
fn load_manifest(pkg_dir: &Path) -> Option<MusiManifest> {
    let manifest_path = pkg_dir.join("mspackage.toml");
    musi_manifest::load(&manifest_path).ok()
}

fn resolve_entry_point(pkg_dir: &Path, manifest: Option<&MusiManifest>) -> PathBuf {
    if let Some(m) = manifest {
        let main = &m.package.main;
        if !main.is_empty() {
            return pkg_dir.join(main);
        }
    }
    pkg_dir.join("index.ms")
}

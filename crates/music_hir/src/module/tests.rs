...
fn store_allocates_and_round_trips_ids() {
    ...
    let lit = store.alloc_lit(HirLit::new(origin, HirLitKind::Int { raw: "1".into() }));
    let expr = store.alloc_expr(HirExpr::new(origin, HirExprKind::Lit { lit }));
...
fn module_wraps_store_and_root() {
    ...
    let root = store.alloc_expr(HirExpr::new(origin, HirExprKind::Error));

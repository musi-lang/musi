...
fn line_col_at_newline_boundary() {
    ...
    let src = map.get(id).unwrap();
    // '\n' is at offset 1, logically part of line 1
    assert_eq!(src.line_col(1), (1, 2));
    // 'b' is at offset 2, start of line 2
...
fn line_count_trailing_newline() {
    ...
    let id = map.add("test.ms", "a\n");
    let src = map.get(id).unwrap();
    // "a\n" => lines: line 1 "a", line 2 "" (empty after newline)
...
fn line_col_at_eof() {
    ...
    let id = map.add("test.ms", "hello");
    let src = map.get(id).unwrap();
    // offset 5 == text.len(), should be clamped and report end of line 1
...
fn line_col_past_eof() {
    ...
    let id = map.add("test.ms", "hello");
    let src = map.get(id).unwrap();
    // offset 100 is well past EOF, should be clamped to text.len() (5)

...
//!
//! The ISA collapses INT/NAT/FLT typed opcodes into a single `ADD`/`SUB`/etc.
//! set. At runtime we check the NaN-box tag: both floats → f64 op, otherwise
//! → i64 via `as_int_wide` (which handles heap-boxed values).
...
///
/// Both floats → f64. Otherwise promotes both to i64 via `as_int_wide`.
...
pub fn exec(op: Opcode, frame: &mut Frame, heap: &mut Heap) -> VmResult<bool> {
    match op {
        ...
        // §4.4 Bitwise (Int — Bool and/or/not compile to branch sequences)
        Opcode::BAND => {
            let (b, a) = frame.pop2()?;
            let result = a.as_int_wide(heap)? & b.as_int_wide(heap)?;
            frame.stack.push(Value::from_int_wide(result, heap));
        }
        Opcode::BOR => {
            let (b, a) = frame.pop2()?;

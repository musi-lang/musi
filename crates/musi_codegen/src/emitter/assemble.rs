...
use super::state::{
    EmitArenas, EmitState, FnEmitter, param_list_with_types, resolve_type_tag, ty_name_str,
};
            ...
            Expr::GivenDef {
                ...
                for member in &members {
                    ...
                    let fn_idx = match module
                        .method_table
                        .iter()
                        .find(|e| e.name.as_ref() == fn_name.as_str() && e.type_tag == type_tag)
                    {
...
pub(super) fn emit_main_body(
    ...
    let arenas = EmitArenas {
        ...
        expr_lists: &user.ctx.expr_lists,
        interner,
    };
    let mut out = FnEmitter::new();
    out.push_scope();
        ...
        match user.ctx.exprs.get(item_idx) {
            Expr::FnDef {
                ...
                if body.is_some() && !modifiers.iter().any(|m| matches!(m, Modifier::Extrin(_))) {
                    ...
                    for attr in attrs {
                        if let (name, Some(&fn_idx), Some(&test_fn_idx)) = (

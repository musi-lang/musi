...
enum Command {
    ...
    /// Add a dependency to mspackage.toml
    Add {
    ...
    /// Run a task from mspackage.toml
    Task {
...
fn main() {
    ...
    match cli.command {
        ...
        Command::Test { filter } => {
            cmd::test::run(filter.as_deref(), manifest.as_ref(), project_root.as_deref());
        }
    }
}
...
fn load_manifest(source_hint: Option<&Path>) -> Option<(MusiManifest, PathBuf)> {
    ...
    loop {
        let candidate = dir.join("mspackage.toml");
        if candidate.exists() {
            let manifest = musi_manifest::load(&candidate).ok()?;

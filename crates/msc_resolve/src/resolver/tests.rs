...
#[test]
fn test_resolve_msc_core_as_directory_index() {
    let dir = temp_dir();
...
#[test]
fn test_resolve_msc_nested_path_as_file() {
    let dir = temp_dir();
    let std_root = dir.path().join("stdlib");
...
#[test]
fn test_resolve_msc_builtin_returns_sentinel_path() {
    let dir = temp_dir();
    let config = make_config(dir.path().join("stdlib"), dir.path().to_path_buf());
...
#[test]
fn test_resolve_msc_non_builtin_returns_error() {
    let dir = temp_dir();
    let config = make_config(dir.path().join("stdlib"), dir.path().to_path_buf());

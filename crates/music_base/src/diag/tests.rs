...
fn sample_expected_message() -> String {
    ["expected", " ';' after expression"].concat()
}
...
fn sample_plain_message() -> String {
    ["something", " went wrong"].concat()
}

fn sample_hint_message() -> String {
    ["undefined", " binding 'writein'"].concat()
}
...
mod success {
    ...
    fn builder_chain() {
        ...
        let message = sample_expected_message();
        let label = sample_label();
        let diag = Diag::error(message.clone())
            .with_code(DiagCode::new(2001))
            .with_label(Span::new(16, 17), source_id, label.clone())
        ...
        assert_eq!(diag.labels()[0].message(), label);
    ...
    fn emit_colorless_matches_expected_format() {
        ...
        let expected = concat!("src/main.ms:1:14: ", "error", "[MS", "2001]: ").to_owned()
            + message.as_str()
            + concat!(
                "\n",
                "  |\n",
    ...
    fn no_labels_just_message() {
        ...
        let message = sample_plain_message();
        let diag = Diag::error(message.clone()).with_code(DiagCode::new(2999));

        let output = emit_to_string(&diag, &sources, false);

        assert_eq!(output, format!("error[{}2999]: {message}\n", "MS"));
    }
    ...
    fn hint_renders_as_help_line() {
        let (sources, source_id) = make_source_map();
        let message = sample_hint_message();
        let diag = Diag::error(message.clone())
        ...
        let output = emit_to_string(&diag, &sources, false);
        assert!(output.contains(format!("error[{}3001]: {message}", "MS").as_str()));

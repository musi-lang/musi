...
use music_sema::{
    Attr, AttrValue, DefinitionKey, SemaModule, SurfaceEffectRow, SurfaceTy, SurfaceTyId,
    SurfaceTyKind,
};
...
use crate::api::{
    ...
    IrHandleOp, IrInstanceDef, IrLit, IrMetaRecord, IrModule, IrNameRef, IrOrigin, IrParam,
...
fn format_attr(attr: &Attr) -> String {
    ...
    let args = attr
        ...
        .map(|arg| match arg.name.as_deref() {
            Some(name) => format!("{name} := {}", format_attr_value(&arg.value)),
            None => format_attr_value(&arg.value),
        })
        .collect::<Vec<_>>()
        .join(", ");
    format!("@{path}({args})")
...
fn collect_meta(sema: &SemaModule) -> Box<[IrMetaRecord]> {
    ...
    for class in &surface.exported_classes {
        ...
        for attr in class.inert_attrs.iter() {
            out.push(IrMetaRecord {
                target: qualified_name(&class.key.module, class.key.name.as_ref()),
                key: "inert.attr".into(),
        ...
        for attr in class.musi_attrs.iter() {
            out.push(IrMetaRecord {
                target: qualified_name(&class.key.module, class.key.name.as_ref()),
                key: "musi.attr".into(),
                values: vec![format_attr(attr).into_boxed_str()].into_boxed_slice(),
    ...
    for effect in &surface.exported_effects {
        ...
        for attr in effect.inert_attrs.iter() {
            out.push(IrMetaRecord {
                target: qualified_name(&effect.key.module, effect.key.name.as_ref()),
                key: "inert.attr".into(),
                values: vec![format_attr(attr).into_boxed_str()].into_boxed_slice(),
        ...
        for attr in effect.musi_attrs.iter() {
            out.push(IrMetaRecord {
                target: qualified_name(&effect.key.module, effect.key.name.as_ref()),
                key: "musi.attr".into(),
                values: vec![format_attr(attr).into_boxed_str()].into_boxed_slice(),
    ...
    for data in &surface.exported_data {
        for attr in data.inert_attrs.iter() {
            out.push(IrMetaRecord {
        ...
        for attr in data.musi_attrs.iter() {
            out.push(IrMetaRecord {
                target: qualified_name(&data.key.module, data.key.name.as_ref()),
                key: "musi.attr".into(),
                values: vec![format_attr(attr).into_boxed_str()].into_boxed_slice(),
    ...
    for export in &surface.exported_values {
        ...
        for attr in export.inert_attrs.iter() {
            out.push(IrMetaRecord {
                target: target.clone(),
                key: "inert.attr".into(),
                values: vec![format_attr(attr).into_boxed_str()].into_boxed_slice(),
        ...
        for attr in export.musi_attrs.iter() {
            out.push(IrMetaRecord {
                target: target.clone(),
                key: "musi.attr".into(),
                values: vec![format_attr(attr).into_boxed_str()].into_boxed_slice(),
    ...
    for (idx, inst) in surface.exported_instances.iter().enumerate() {
        ...
        for attr in inst.inert_attrs.iter() {
            out.push(IrMetaRecord {
                target: target.clone(),
                key: "inert.attr".into(),
                values: vec![format_attr(attr).into_boxed_str()].into_boxed_slice(),
        ...
        for attr in inst.musi_attrs.iter() {
            out.push(IrMetaRecord {
                target: target.clone(),
                key: "musi.attr".into(),
                values: vec![format_attr(attr).into_boxed_str()].into_boxed_slice(),

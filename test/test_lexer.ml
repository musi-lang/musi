...
let test_char_preds () =
  ...
  (check bool) "is_space space" true (Lexer.is_space ' ');
  (check bool) "is_space tab" true (Lexer.is_space '\t');
  (check bool) "is_space newline" false (Lexer.is_space '\n');
...
let test_numbers_decimal () =
  ...
  let new_state, content = Lexer.scan_number state in
...
let test_numbers_errors_incomplete_hex () =
  ...
  let new_state, _content = Lexer.scan_number state in
  (check bool)
...
let test_comments_block () =
  ...
  (check int) "block comment position" 13 new_state.pos

let test_whitespace_space () =
  let state, _ = make_test_state "   " in
  let new_state, _span = Lexer.scan_ws state in

...
- Phase DAG (no cycles): `music_base -> music_names -> music_syntax -> music_module -> music_resolve -> music_sema -> music_ir -> music_bc -> music_assembly -> music_codegen -> music_session` (project integration lives above, in `musi_project`/`music_session`). Pre-resolve contracts use `*Env` names, not `*Resolver`.
